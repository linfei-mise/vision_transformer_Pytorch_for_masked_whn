...
def read_split_data(root: str, val_rate: float = 0.1):  # 划分率为9：1
...
def train_one_epoch(model, optimizer, data_loader, device, epoch):  # 多GPU并行训练
    model.train()
...
def plot_class_preds(net,  # 传入tensorboard图片
                     images_dir: str,
                     transform,
                     num_plot: int = 5,
                     device="cpu"):
